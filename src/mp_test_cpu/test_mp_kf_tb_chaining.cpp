...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
		...
		cout << "+-----+   +-----+   +------+   +-----+   +-------+   +-----+" << endl;
	    // prepare the test
	    PipeGraph graph("test_kf_tb_ch", Mode::DETERMINISTIC);
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    Source source = Source_Builder(source_functor)
	    					.withName("test_kf_tb_ch_source")
	    ...
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor)
	    						.withName("test_kf_tb_ch_flatmap")
	    ...
	    Map map = Map_Builder(map_functor)
	    				.withName("test_kf_tb_ch_map")
	    				.withParallelism(degree2)
	    				.build();
	    mp.chain(map);
	    // kf
	    Key_Farm kf = KeyFarm_Builder(kf_function)
	    					.withName("test_kf_tb_ch_kf")
