 ...
 *  
 *  Composition: Source -> Filter -> FlatMap -> Map -> PF_TB -> Sink
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor)
	    						.withName("test_pf_tb_ch_flatmap")
	    ...
	    Sink sink = Sink_Builder(sink_functor)
	    					.withName("test_pf_tb_chtest_pf_tb_ch_sink")
	    					.withParallelism(1)
	    					.build();
