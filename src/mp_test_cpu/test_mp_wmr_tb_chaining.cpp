 ...
 *  
 *  Composition: Source -> Filter -> FlatMap -> Map -> WMR_TB -> Sink
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // prepare the test
	    PipeGraph graph("test_wmr_tb_ch");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    Source source = Source_Builder(source_functor)
	    					.withName("test_wmr_tb_ch_source")
	    ...
	    Filter filter = Filter_Builder(filter_functor)
	    						.withName("test_wmr_tb_ch_filter")
	    ...
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor)
	    						.withName("test_wmr_tb_ch_flatmap")
