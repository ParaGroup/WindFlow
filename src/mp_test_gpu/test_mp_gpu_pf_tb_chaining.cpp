...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    cout << "Run " << i << endl;	
		cout << "+-----+   +-----+   +------+   +-----+   +-------+   +-----+" << endl;
		cout << "|  S  |   |  F  |   |  FM  |   |  M  |   | PF_TB |   |  S  |" << endl;
		cout << "| (" << source_degree << ") +-->+ (" << degree1 << ") +-->+  (" << degree2 << ") +-->+ (" << degree2 << ") +-->+ (" << plq_degree << "," << wlq_degree << ") +-->+ (1) |" << endl;
		cout << "+-----+   +-----+   +------+   +-----+   +-------+   +-----+" << endl;
	    // prepare the test
	    PipeGraph graph("test_pf_tb_gpu_ch", Mode::DETERMINISTIC);
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    auto *source = Source_Builder<decltype(source_functor)>(source_functor)
	    						.withName("test_pf_tb_gpu_ch_source")
	    ...
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor)
	    						.withName("test_pf_tb_gpu_ch_filter")
	    ...
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor)
	    							.withName("test_pf_tb_gpu_ch_flatmap")
	    ...
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor)
	    						.withName("test_pf_tb_gpu_ch_map")
	    						.withParallelism(degree2)
	    						.build_ptr();
	    mp.chain(*map);
	    // pf
	    ...
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor)
	    						.withName("test_pf_tb_gpu_ch_sink")
	    						.withParallelism(1)
	    						.build_ptr();
	    mp.chain_sink(*sink);
	   	// run the application
