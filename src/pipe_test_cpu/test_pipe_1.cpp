...
public:
	// constructor
	Source_Functor(size_t _stream_len): stream_len(_stream_len), i(0) {}
	...
	void operator()(Shipper<tuple_t> &shipper)
	{
		...
		LOCKED_PRINT("Generated " << i << " items" << endl;)
	}
};
...
int main(int argc, char *argv[])
{
	...
	if (argc != 7) {
		cout << argv[0] << " -l [stream_length] -n [sources] -m [sinks]" << endl;
		exit(EXIT_SUCCESS);
	}
    ...
    Sink sink = Sink_Builder(sink_functor).withName("test1_sink").withParallelism(pardegree_sink).build();
