 ...
 *  
 *  Composition: Source(1) -> Filter(*) -> FlatMap(*) -> Map(*) -> KF_TB(*) -> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
    	...
    	kf_degree = dist6(rng);
    	cout << "Run " << i << " Source(" << source_degree <<")->Filter(" << filter_degree << ")->FlatMap(" << flatmap_degree << ")->Map(" << map_degree << ")->Key_Farm_TB(" << kf_degree << ")->Sink(1)" << endl;
	    // prepare the test
	    MultiPipe application("test_kf_tb");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    Source source = Source_Builder(source_functor).withName("test_kf_tb_source").withParallelism(source_degree).build();
	    application.add_source(source);
	    // filter
	    Filter_Functor filter_functor;
	    Filter filter = Filter_Builder(filter_functor).withName("test_kf_tb_filter").withParallelism(filter_degree).build();
	    application.add(filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor).withName("test_kf_tb_flatmap").withParallelism(flatmap_degree).build();
	    application.add(flatmap);
	    // map
	    Map_Functor map_functor;
	    Map map = Map_Builder(map_functor).withName("test_kf_tb_map").withParallelism(map_degree).build();
	    application.add(map);
	    // kf
	    Key_Farm kf = KeyFarm_Builder(kf_function).withName("test_kf_tb_kf").withParallelism(kf_degree).withTBWindows(microseconds(win_len), microseconds(win_slide)).build();
	    application.add(kf);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    Sink sink = Sink_Builder(sink_functor).withName("test_kf_tb_sink").withParallelism(1).build();
