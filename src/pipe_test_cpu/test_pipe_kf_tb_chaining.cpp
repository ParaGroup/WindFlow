...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    Filter filter = Filter_Builder(filter_functor).withName("test_kf_tb_ch_filter").withParallelism(degree1).build();
	    application.add(filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor).withName("test_kf_tb_ch_flatmap").withParallelism(degree2).build();
	    ...
	    Sink sink = Sink_Builder(sink_functor).withName("test_kf_tb_ch_sink").withParallelism(1).build();
