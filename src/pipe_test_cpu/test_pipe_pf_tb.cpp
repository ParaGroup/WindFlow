...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    application.add(map);
	    // pf
	    Pane_Farm pf = PaneFarm_Builder(plq_function, wlq_function).withName("test_pf_tb_pf").withParallelism(plq_degree, wlq_degree).withTBWindow(microseconds(win_len), microseconds(win_slide)).build();
	    application.add(pf);
	    // sink
