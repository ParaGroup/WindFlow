...
int wf_function(size_t key, size_t wid, Iterable<tuple_t> &input, output_t &result) {
	long sum = 0;
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    application.add(map);
	    // wf
	    Win_Farm wf = WinFarm_Builder(wf_function).withName("test_wf_cb_wf").withParallelism(wf_degree).withCBWindow(win_len, win_slide).build();
	    application.add(wf);
	    // sink
