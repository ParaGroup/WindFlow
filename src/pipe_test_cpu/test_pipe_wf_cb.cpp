 ...
 *  
 *  Composition: Source(1) -> Filter(*) -> FlatMap(*) -> Map(*) -> WF_CB(*) -> Sink(1)
 */ 
...
void wf_function(size_t wid, Iterable<tuple_t> &input, output_t &result) {
	long sum = 0;
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
    	...
    	wf_degree = dist6(rng);
    	cout << "Run " << i << " Source(" << source_degree <<")->Filter(" << filter_degree << ")->FlatMap(" << flatmap_degree << ")->Map(" << map_degree << ")->Win_Farm_CB(" << wf_degree << ")->Sink(1)" << endl;
	    // prepare the test
	    MultiPipe application("test_wf_cb");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    Source source = Source_Builder(source_functor).withName("test_wf_cb_source").withParallelism(source_degree).build();
	    application.add_source(source);
	    // filter
	    Filter_Functor filter_functor;
	    Filter filter = Filter_Builder(filter_functor).withName("test_wf_cb_filter").withParallelism(filter_degree).build();
	    application.add(filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor).withName("test_wf_cb_flatmap").withParallelism(flatmap_degree).build();
	    application.add(flatmap);
	    // map
	    Map_Functor map_functor;
	    Map map = Map_Builder(map_functor).withName("test_wf_cb_map").withParallelism(map_degree).build();
	    application.add(map);
	    // wf
	    Win_Farm wf = WinFarm_Builder(wf_function).withName("test_wf_cb_wf").withParallelism(wf_degree).withCBWindows(win_len, win_slide).build();
	    application.add(wf);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    Sink sink = Sink_Builder(sink_functor).withName("test_wf_cb_sink").withParallelism(1).build();
