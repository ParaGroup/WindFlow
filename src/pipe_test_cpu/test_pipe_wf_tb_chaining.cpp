...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // wf
	    Win_Farm wf = WinFarm_Builder(wf_function).withName("test_wf_tb_ch_wf").withParallelism(wf_degree).withTBWindow(microseconds(win_len), microseconds(win_slide)).build();
	    application.add(wf);
	    // sink
