 ...
 *  
 *  Composition: Source(1) -> Filter(*) -?-> FlatMap(*) -c-> Map(*) -> WMR_CB(*) -?-> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // prepare the test
	    Pipe application("test_wmr_cb_ch");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    Source source = Source_Builder(source_functor).withName("test_wmr_cb_ch_source").withParallelism(source_degree).build();
	    application.add_source(source);
	    // filter
	    Filter_Functor filter_functor;
	    Filter filter = Filter_Builder(filter_functor).withName("test_wmr_cb_ch_filter").withParallelism(degree1).build();
	    application.add(filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    FlatMap flatmap = FlatMap_Builder(flatmap_functor).withName("test_wmr_cb_ch_flatmap").withParallelism(degree2).build();
	    application.chain(flatmap);
	    // map
	    Map_Functor map_functor;
	    Map map = Map_Builder(map_functor).withName("test_wmr_cb_ch_map").withParallelism(degree2).build();
	    application.chain(map);
	    // wmr
	    Win_MapReduce wmr = WinMapReduce_Builder(wmap_function, reduce_function).withName("test_wmr_cb_ch_wmr").withParallelism(wmap_degree, reduce_degree).withCBWindow(win_len, win_slide).build();
	    application.add(wmr);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    Sink sink = Sink_Builder(sink_functor).withName("test_wmr_cb_ch_sink").withParallelism(1).build();
