...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // wmr
	    Win_MapReduce wmr = WinMapReduce_Builder(wmap_function, reduce_function).withName("test_wmr_cb_ch_wmr").withParallelism(wmap_degree, reduce_degree).withCBWindows(win_len, win_slide).build();
	    application.add(wmr);
	    // sink
