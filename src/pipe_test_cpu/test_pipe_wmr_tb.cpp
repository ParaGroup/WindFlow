...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    application.add(map);
	    // wmr
	    Win_MapReduce wmr = WinMapReduce_Builder(wmap_function, reduce_function).withName("test_wmr_tb_wmr").withParallelism(wmap_degree, reduce_degree).withTBWindow(microseconds(win_len), microseconds(win_slide)).build();
	    application.add(wmr);
	    // sink
