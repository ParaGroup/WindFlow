 ...
 *  
 *  Composition: Source(1) -> Filter(*) -> FlatMap(*) -> Map(*) -> KF_GPU_TB(WMR_GPU_TB(*, *), *) -> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    int filter_degree, flatmap_degree, map_degree, kf_degree = 1, wmap_degree = 1, reduce_degree = 1;
    ...
    for (size_t i=0; i<runs; i++) {
    	...
    	cout << "Run " << i << " Source(" << source_degree <<")->Filter(" << filter_degree << ")->FlatMap(" << flatmap_degree << ")->Map(" << map_degree << ")->Key_Farm_GPU_TB(Win_MapReduce_GPU_TB(" << wmap_degree << ", " << reduce_degree << "), " << kf_degree << ")->Sink(1)" << endl;
	    // prepare the test
	    MultiPipe application("test_kf+wmr_tb_gpu");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    auto *source = Source_Builder<decltype(source_functor)>(source_functor).withName("test_kf+wmr_tb_gpu_source").withParallelism(source_degree).build_ptr();
	    application.add_source(*source);
	    // filter
	    Filter_Functor filter_functor;
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor).withName("test_kf+wmr_tb_gpu_filter").withParallelism(filter_degree).build_ptr();
	    application.add(*filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor).withName("test_kf+wmr_tb_gpu_flatmap").withParallelism(flatmap_degree).build_ptr();
	    application.add(*flatmap);
	    // map
	    Map_Functor map_functor;
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor).withName("test_kf+wmr_tb_gpu_map").withParallelism(map_degree).build_ptr();
	    application.add(*map);
	    // wmr
	    ...
	    auto *wmr = WinMapReduceGPU_Builder<decltype(map_function), decltype(reduce_function)>(map_function, reduce_function).withName("test_wmr_tb_wmr").withParallelism(wmap_degree, reduce_degree).withTBWindows(microseconds(win_len), microseconds(win_slide)).withBatch(batch_len).withOptLevel(LEVEL2).build_ptr();
	    // kf
	   	auto *kf = KeyFarmGPU_Builder<decltype(*wmr)>(*wmr).withName("test_kf+wmr_tb_kf").withParallelism(kf_degree).build_ptr();
	    application.add(*kf);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor).withName("test_kf+wmr_tb_gpu_sink").withParallelism(1).build_ptr();
