 ...
 *  
 *  Composition: Source(1) -> Filter(*) -?-> FlatMap(*) -c-> Map(*) -> KF_GPU_CB(*) -?-> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
    	...
    	degree2 = dist6(rng);
    	kf_degree = dist6(rng);
    	cout << "Run " << i << " Source(" << source_degree <<")->Filter(" << degree1 << ")-?->FlatMap(" << degree2 << ")-c->Map(" << degree2 << ")->Key_Farm_GPU_CB(" << kf_degree << ")-?->Sink(1)" << endl;
	    // prepare the test
	    MultiPipe application("test_kf_cb_gpu_ch");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    auto *source = Source_Builder<decltype(source_functor)>(source_functor).withName("test_kf_cb_gpu_ch_source").withParallelism(source_degree).build_ptr();
	    application.add_source(*source);
	    // filter
	    Filter_Functor filter_functor;
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor).withName("test_kf_cb_gpu_ch_filter").withParallelism(degree1).build_ptr();
	    application.add(*filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor).withName("test_kf_cb_gpu_ch_flatmap").withParallelism(degree2).build_ptr();
	    ...
	    // kf
	    auto *kf = KeyFarmGPU_Builder<decltype(kf_function)>(kf_function).withName("test_kf_cb_gpu_ch_kf").withParallelism(kf_degree).withCBWindows(win_len, win_slide).withBatch(batch_len).build_ptr();
	    application.add(*kf);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor).withName("test_kf_cb_gpu_ch_sink").withParallelism(1).build_ptr();
