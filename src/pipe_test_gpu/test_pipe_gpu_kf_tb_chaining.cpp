...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // prepare the test
	    Pipe application("test_kf_tb_gpu_ch");
	    // source
	    ...
	    // kf
	    auto *kf = KeyFarmGPU_Builder<decltype(kf_function)>(kf_function).withName("test_kf_tb_gpu_ch_kf").withParallelism(kf_degree).withTBWindow(microseconds(win_len), microseconds(win_slide)).withBatch(batch_len).build_ptr();
	    application.add(*kf);
	    // sink
