 ...
 *  
 *  Composition: Source(1) -> Filter(*) -> FlatMap(*) -> Map(*) -> PF_GPU_CB(*) -> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // prepare the test
	    Pipe application("test_pf_cb_gpu");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    auto *source = Source_Builder<decltype(source_functor)>(source_functor).withName("test_pf_cb_gpu_source").withParallelism(source_degree).build_ptr();
	    application.add_source(*source);
	    // filter
	    Filter_Functor filter_functor;
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor).withName("test_pf_cb_gpu_filter").withParallelism(filter_degree).build_ptr();
	    application.add(*filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor).withName("test_pf_cb_gpu_flatmap").withParallelism(flatmap_degree).build_ptr();
	    application.add(*flatmap);
	    // map
	    Map_Functor map_functor;
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor).withName("test_pf_cb_gpu_map").withParallelism(map_degree).build_ptr();
	    application.add(*map);
	    ...
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor).withName("test_pf_cb_gpu_sink").withParallelism(1).build_ptr();
