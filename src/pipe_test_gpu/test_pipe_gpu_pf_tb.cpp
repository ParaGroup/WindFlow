...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // pf
	    auto *pf = PaneFarmGPU_Builder<decltype(plq_function), decltype(wlq_function)>(plq_function, wlq_function).withName("test_pf_tb_pf").withParallelism(plq_degree, wlq_degree).withTBWindow(microseconds(win_len), microseconds(win_slide)).withBatch(batch_len).build_ptr();
	    application.add(*pf);
	    // sink
