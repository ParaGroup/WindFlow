...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor).withName("test_wf_tb_gpu_filter").withParallelism(filter_degree).build_ptr();
	    application.add(*filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor).withName("test_wf_tb_gpu_flatmap").withParallelism(flatmap_degree).build_ptr();
	    application.add(*flatmap);
	    // map
	    Map_Functor map_functor;
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor).withName("test_wf_tb_gpu_map").withParallelism(map_degree).build_ptr();
	    application.add(*map);
	    ...
	    // wf
	    auto *wf = WinFarmGPU_Builder<decltype(wf_function)>(wf_function).withName("test_wf_tb_gpu_wf").withParallelism(wf_degree).withTBWindows(microseconds(win_len), microseconds(win_slide)).withBatch(batch_len).build_ptr();
	    application.add(*wf);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor).withName("test_wf_tb_gpu_sink").withParallelism(1).build_ptr();
