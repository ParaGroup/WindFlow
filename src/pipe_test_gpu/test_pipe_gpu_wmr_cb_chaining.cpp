 ...
 *  
 *  Composition: Source(1) -> Filter(*) -?-> FlatMap(*) -c-> Map(*) -> WMR_GPU_CB(*) -?-> Sink(1)
 */ 
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    // prepare the test
	    MultiPipe application("test_wmr_cb_gpu_ch");
	    // source
	    Source_Functor source_functor(stream_len, n_keys);
	    auto *source = Source_Builder<decltype(source_functor)>(source_functor).withName("test_wmr_cb_gpu_ch_source").withParallelism(source_degree).build_ptr();
	    application.add_source(*source);
	    // filter
	    Filter_Functor filter_functor;
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor).withName("test_wmr_cb_gpu_ch_filter").withParallelism(degree1).build_ptr();
	    application.add(*filter);
	    // flatmap
	    FlatMap_Functor flatmap_functor;
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor).withName("test_wmr_cb_gpu_ch_flatmap").withParallelism(degree2).build_ptr();
	    application.chain(*flatmap);
	    // map
	    Map_Functor map_functor;
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor).withName("test_wmr_cb_gpu_ch_map").withParallelism(degree2).build_ptr();
	    ...
	    // wmr
	    auto *wmr = WinMapReduceGPU_Builder<decltype(map_function), decltype(reduce_function)>(map_function, reduce_function).withName("test_wmr_cb_wmr_ch").withParallelism(wmap_degree, reduce_degree).withCBWindows(win_len, win_slide).withBatch(batch_len).build_ptr();
	    application.add(*wmr);
	    // sink
	    Sink_Functor sink_functor(n_keys);
	    auto *sink = Sink_Builder<decltype(sink_functor)>(sink_functor).withName("test_wmr_cb_gpu_ch_sink").withParallelism(1).build_ptr();
