 ...
 *  of patterns instantiated with count-based windows.
 */	
...
int main(int argc, char *argv[])
{
	...
	if (argc != 19) {
		cout << argv[0] << " -l [stream_length] -k [num keys] -w [win length] -s [win slide] -e [pardegree ] -r [pardegree] -n [pardegree] -m [pardegree] -o [opt_level]" << endl;
		exit(EXIT_SUCCESS);
	}
	...
	if (degree2 > 1) {
		...
		Win_MapReduce wmNICNIC = WinMapReduce_Builder(wmFNIC, wmFNIC).withCBWindow(win_len, win_slide)
											   ...
											   .withOpt(opt_level)
											   .build();
		Win_Farm wf = WinFarm_Builder(wmNICNIC).withCBWindow(win_len, win_slide)
											   .withParallelism(degree1)
