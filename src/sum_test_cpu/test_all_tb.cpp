...
int main(int argc, char *argv[])
{
	...
	// Test 1 SEQ(NIC)
	{
		...
		Win_Seq seqNIC = WinSeq_Builder(seqFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withName("seq_nic")
											   .build();
	...
	// Test 2 SEQ(INC)
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_Seq seqINC = WinSeq_Builder(seqFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withName("seq_inc")
											   .build();
	...
	// Test 3 WF(SEQ(NIC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_Farm wfNIC = WinFarm_Builder(seqFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
	...
	// Test 4 WF(SEQ(INC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_Farm wfINC = WinFarm_Builder(seqFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree1)
											   .withName("wf_inc")
											   .withEmitters(emitter_degree)
											   .build();
	...
	// Test 5 KF(SEQ(NIC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Key_Farm kfNIC = KeyFarm_Builder(seqFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree1)
											   .withName("kf_nic")
											   .build();
	...
	// Test 6 KF(SEQ(INC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Key_Farm kfINC = KeyFarm_Builder(seqFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree1)
											   .withName("kf_inc")
											   .build();
	...
	// Test 7 PF(PLQ(NIC), WLQ(NIC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Pane_Farm pfNICNIC = PaneFarm_Builder(plqFNIC, wlqFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
	...
	// Test 8 PF(PLQ(NIC), WLQ(INC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Pane_Farm pfNICINC = PaneFarm_Builder(plqFNIC, wlqFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("pf_nic_inc")
											   .withOptLevel(opt_level)
											   .build();
	...
	// Test 9 PF(PLQ(INC), WLQ(NIC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Pane_Farm pfINCNIC = PaneFarm_Builder(plqFINC, wlqFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("pf_inc_nic")
											   .withOptLevel(opt_level)
											   .build();
	...
	// Test 10 PF(PLQ(INC), WLQ(INC))
	{
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Pane_Farm pfINCINC = PaneFarm_Builder(plqFINC, wlqFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("pf_inc_inc")
											   .withOptLevel(opt_level)
											   .build();
	...
	if (degree2 > 1) {
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_MapReduce wmNICNIC = WinMapReduce_Builder(wmFNIC, wmFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("wm_nic_nic")
											   .withOptLevel(opt_level)
											   .build();
	...
	if (degree2 > 1) {
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_MapReduce wmNICINC = WinMapReduce_Builder(wmFNIC, wmFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("wm_nic_inc")
											   .withOptLevel(opt_level)
											   .build();
	...
	if (degree2 > 1) {
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_MapReduce wmINCNIC = WinMapReduce_Builder(wmFINC, wmFNIC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("wm_inc_nic")
											   .withOptLevel(opt_level)
											   .build();
	...
	if (degree2 > 1) {
		Generator generator(stream_len, num_keys);
		Consumer consumer(num_keys);
		Win_MapReduce wmINCINC = WinMapReduce_Builder(wmFINC, wmFINC).withTBWindows(microseconds(win_len), microseconds(win_slide))
											   .withParallelism(degree2, degree3)
											   .withName("wm_inc_inc")
											   .withOptLevel(opt_level)
											   .build();
