...
/*  
 *  Test Program of the nesting between Win_Farm and Win_MapReduce (TB windows with an
 *  Incremental Query)
 *  
...
int main(int argc, char *argv[])
{
	...
	if (argc != 15) {
		cout << argv[0] << " -l [stream_length] -k [num keys] -w [win length usec] -s [win slide usec] -r [WF pardegree] -n [MAP pardegree] -m [REDUCE pardegree]" << endl;
		exit(EXIT_SUCCESS);
	}
