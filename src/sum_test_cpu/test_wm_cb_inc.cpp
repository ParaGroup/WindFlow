...
/*  
 *  Test Program of the Win_MapReduce Pattern (CB windows with an Incremental Query)
 *  
 *  Test program of the Win_MapReduce pattern instantiated with an incremental query.
...
int main(int argc, char *argv[])
{
	...
	if (argc != 13) {
		cout << argv[0] << " -l [stream_length] -k [num keys] -w [win length] -s [win slide] -n [MAP pardegree] -m [REDUCE pardegree]" << endl;
		exit(EXIT_SUCCESS);
	}
