...
/*  
 *  Test Program of the nesting between Key_Farm_GPU and Win_MapReduce_GPU (CB windows with
 *  REDUCE phase on the GPU)
 *  
 *  Test program of the nesting between the Key_Farm_GPU and the Win_MapReduce_GPU pattern
 *  instantiated with a non-incremental MAP phase on the CPU while the REDUCE phase in on GPU.
...
int main(int argc, char *argv[])
{
	...
	// user-defined map function (Non-Incremental Query)
	...
	auto G = [] __host__ __device__ (size_t wid, const tuple_t *data, tuple_t *res, size_t size, char *memory) {
		long sum = 0;
	...
	auto *wm_gpu = WinMapReduceGPU_Builder<decltype(F), decltype(G)>(F, G).withTBWindows(microseconds(win_len), microseconds(win_slide))
	...
	auto *kf_gpu = KeyFarmGPU_Builder<decltype(*wm_gpu)>(*wm_gpu).withParallelism(kf_degree)
													             .withName("test_sum")
													             .withOptLevel(LEVEL)
													             .build_ptr();
