...
/*  
 *  Test Program of the Win_MapReduce_GPU Pattern (CB windows with REDUCE stage on GPU)
 *  
 *  Test program of the Win_MapReduce_GPU pattern instantiated with a non-incremental function for
 *  the MAP stage on the CPU while the second stage (REDUCE) is offloaded on a GPU device. The query
...
int main(int argc, char *argv[])
{
	...
	if (argc != 15) {
		cout << argv[0] << " -l [stream_length] -k [num keys] -w [win length usec] -s [win slide usec] -n [MAP pardegree] -m [REDUCE pardegree] -b [batch len]" << endl;
		exit(EXIT_SUCCESS);
	}
