...
#ifndef RAW_YSB_NODES_BUF
#define RAW_YSB_NODES_BUF
...
public:
    // constructor
    EventSource(unsigned long _time_sec, unsigned long **_ads_arrays, unsigned int _adsPerCampaign, unsigned int _buffered_cnt=1):
                execution_time_sec(_time_sec), ads_arrays(_ads_arrays), adsPerCampaign(_adsPerCampaign), num_sent(0), buffered_cnt(_buffered_cnt), count(0) {}
    ...
    event_t *svc(event_t *)
    {
        ...
        while (true) {
            ...
            if (elapsed_time_sec >= execution_time_sec)
                ...
                if (count == 0) {
                    data = new event_t[buffered_cnt];
                    out = &(data[0]);
                }
                if (count == buffered_cnt) {
                    data = new event_t[buffered_cnt];
                    out = &(data[0]);
                    count = 0;
                }
...
public:
    // constructor
    RawFilter(unsigned int _event_type=0, unsigned int _buffered_cnt=1):
           event_type(_event_type), received(0), pruned(0), buffered_cnt(_buffered_cnt), count(0), first(nullptr) {}
    ...
    void svc_end()
    {
        //cout << "[Filter] pruned " << (100.0 * pruned) / received << "/% of the input tuples" << endl;
