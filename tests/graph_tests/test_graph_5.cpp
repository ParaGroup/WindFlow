...
/*  
 *  Test 5 of general graphs of operators.
 *  
 *  +---------------------+                         +-----------+
 *  |  +-----+   +-----+  |                         |  +-----+  |
 *  |  |  S  |   |  M  |  |                         |  |  S  |  |
...
int main(int argc, char *argv[])
{
    ...
    int map1_degree, map2_degree, flatmap1_degree, filter_degree, sink1_degree, sink2_degree, sink3_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "|  +-----+   +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   | FM  |   |  S  |  |" << endl;
        cout << "|  | (" << source3_degree << ") +-->+ (" << flatmap1_degree << ") +-->+ (" << sink3_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+   +-----+  |" << endl;
        cout << "+-------------------------------+" << endl;
        ...
        if (source3_degree != flatmap1_degree) {
            check_degree += flatmap1_degree;
        }
        if (flatmap1_degree != sink3_degree) {
            check_degree += sink3_degree;
        }
        // prepare the test
        PipeGraph graph("test_graph_5 (DEFAULT)", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        ...
        Source_Positive_Functor source_functor_positive2(stream_len, n_keys, true);
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        // prepare the test
        PipeGraph graph("test_graph_5 (DETERMINISTIC)", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
        // prepare the first MultiPipe
        ...
        pipe5.chain_sink(sink2);
        // prepare the sixth MultiPipe
        Source_Positive_Functor source_functor_positive2(stream_len, n_keys, false);
        Source source3 = Source_Builder(source_functor_positive2)
                            .withName("source3")
                            .withParallelism(source3_degree)
                            .build();
