 ...
 *  and a controlled delay of the input stream.
 */ 
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 15) {
        cout << argv[0] << " -n [num sources] -w [win length usec] -s [win slide usec] -b [batch size] -r [num wins per batch] -a [aggregation] -d [avg delay usec]" << endl;
        exit(EXIT_SUCCESS);
    }
    while ((option = getopt(argc, argv, "n:w:s:b:r:a:d:")) != -1) {
        switch (option) {
    ...
    PipeGraph graph("test_synth_gpu_delayed", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);

    Source_Functor_Delayed source_functor(buffer, BUFFER_SIZE, app_start_time, d);
