...
int main(int argc, char *argv[])
{
    ...
    if (argc != 15) {
        cout << argv[0] << " -n [num sources] -k [n_keys] -w [win length usec] -s [win slide usec] -b [batch size] -r [num wins per batch] -a [aggregation]" << endl;
        exit(EXIT_SUCCESS);
    }
    while ((option = getopt(argc, argv, "n:k:w:s:b:r:a:")) != -1) {
        switch (option) {
    ...
    init_inputBuffer(buffer, BUFFER_SIZE, k);

    // application starting time
    volatile unsigned long app_start_time = current_time_nsecs();

    // prepare the application
    PipeGraph graph("test_synth_gpu_keyed", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
    ...
    if (agg == "sum") {
        ...
        Ffat_Windows_GPU ffat_gpu = Ffat_WindowsGPU_Builder(lift, comb)
                                          .withName("sum_gpu")
                                          .withKeyBy([] __host__ __device__ (const input_t &in) { return in.key; })
