...
/*  
 *  Test 1 of merge between MultiPipes.
 *  
 *  +---------------------+
 *  |  +-----+   +-----+  |
 *  |  |  S  |   |  M  |  |
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "|  +-----+   +-----+   +-----+  |  |    +---------------------+" << endl;
        cout << "|  |  S  |   |  F  |   |  M  |  |  |" << endl;
        cout << "|  | (" << source2_degree << ") +-->+ (" << filter_degree << ") +-->+ (" << map2_degree << ") |  +--+" << endl;
        cout << "|  +-----+   +-----+   +-----+  |" << endl;
        cout << "+-------------------------------+" << endl;
        ...
        if (source2_degree != filter_degree) {
        ...
        // prepare the test
        PipeGraph graph("test_merge_1 (DEFAULT)", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        ...
        Source_Negative_Functor source_functor2(stream_len, n_keys, true);
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        // prepare the test
        PipeGraph graph("test_merge_1 (DETERMINISTIC)", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
        // prepare the first MultiPipe
        ...
        pipe1.chain(map1);
        // prepare the second MultiPipe
        Source_Negative_Functor source_functor2(stream_len, n_keys, false);
        Source source2 = Source_Builder(source_functor2)
                            .withName("source2")
                            .withParallelism(source2_degree)
                            .build();
