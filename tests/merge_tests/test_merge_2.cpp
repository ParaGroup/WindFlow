...
/*  
 *  Test 2 of merge between MultiPipes.
 *  
 *  +---------------------+
 *  |  +-----+   +-----+  |
 *  |  |  S  |   |  M  |  |
...
int main(int argc, char *argv[])
{
    ...
    int map1_degree, map2_degree, filter_degree, map3_degree, flatmap_degree, sink_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "|  +-----+   +-----+  |            |    |  +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   |  M  |  |            |    |  | FM  |   |  S  |  |" << endl;
        cout << "|  | (" << source2_degree << ") +-->+ (" << map2_degree << ") |  +------------+--->+  | (" << flatmap_degree << ") +-->+ (" << sink_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+  |            |    |  +-----+   +-----+  |" << endl;
        cout << "+---------------------+            |    +---------------------+" << endl;
        cout << "                                   |" << endl;
        cout << "+-------------------------------+  |" << endl;
        ...
        // prepare the test
        PipeGraph graph("test_merge_2 (DEFAULT)", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        // prepare the test
        PipeGraph graph("test_merge_2 (DETERMINISTIC)", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
        // prepare the first MultiPipe
