...
/*  
 *  Test 1 of merge between MultiPipes with both CPU and GPU operators. Version
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        // prepare the test
        PipeGraph graph("test_merge_gpu_kb_1", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        ...
        Filter_Functor_GPU_KB filter_functor1;
        Filter_GPU filtergpu = FilterGPU_Builder(filter_functor1)
                                    .withName("filtergpu")
                                    .withParallelism(filter_degree)
                                    .withKeyBy([] __host__ __device__ (const tuple_t &t) -> char { return t.key; })
                                    .build();
        pipe1.chain(filtergpu);
