...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
	    ...
	    auto *filter = Filter_Builder<decltype(filter_functor)>(filter_functor)
	    						.withName("test_kff_tb_gpu_filter")
	    ...
	    auto *flatmap = FlatMap_Builder<decltype(flatmap_functor)>(flatmap_functor)
	    						.withName("test_kff_tb_gpu_flatmap")
	    						.withParallelism(flatmap_degree)
	    						.build_ptr();
	    mp.chain(*flatmap);
	    // map
	    Map_Functor map_functor;
	    auto *map = Map_Builder<decltype(map_functor)>(map_functor).withName("test_kff_tb_gpu_map").withParallelism(map_degree).build_ptr();
	    mp.chain(*map);
	    // kff
