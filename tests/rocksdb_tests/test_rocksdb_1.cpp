...
/*  
 *  Test 1 of general graphs that include persistent operators (using RocksDB).
 *  
 *                              +---------------------+
 *                              |  +-----+   +-----+  |
 *                           +->+  |  F  |   |  M  |  +-+
...
int main(int argc, char *argv[])
{
    ...
    int map1_degree, map2_degree, filter1_degree, filter2_degree, filter3_degree, sink1_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "                            |  +-----+   +-----+  |" << endl;
        cout << "                         +->+  |  F  |   |  M  |  +-+" << endl;
        cout << "                         |  |  | (" << filter2_degree << ") +-->+ (" << map2_degree << ") |  | |" << endl;
        cout << "                         |  |  +-----+   +-----+  | |" << endl;
        cout << "+---------------------+  |  +---------------------+ |   +---------------------+" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   | P_F |  |  |                          |   |  |  M  |   |  S  |  |" << endl;
        cout << "|  | (" << source1_degree << ") +-->+ (" << filter1_degree << ") |  +--+                          +-->+  | (" << map1_degree << ") +-->+ (" << sink1_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "+---------------------+  |       +-----------+      |   +---------------------+" << endl;
        cout << "                         |       |  +-----+  |      |" << endl;
        cout << "                         |       |  |  F  |  |      |" << endl;
        ...
        auto serializer = [](state_t &s) -> std::string {
                            return std::to_string(s.value) + "," + std::to_string(s.count);
        };
        auto deserializer = [](std::string &s) -> state_t {
            state_t state;
            state.value = atoi(s.substr(0, s.find(",")).c_str());
            state.count = atoi(s.substr(s.find(",")+1, s.length()-1).c_str());
            return state;
        };
        // prepare the test
        PipeGraph graph("test_rocksdb_1", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
...
#if 1
        P_Filter_Functor_KB p_filter_functor_kb(2);
        P_Filter p_filter1 = P_Filter_Builder(p_filter_functor_kb)
                                .withName("p_filter1")
                                ...
                                .withDeleteDb(true)
                                .build();
        pipe1.chain(p_filter1);
#else
        Filter_Functor_KB_State filter_functor_kb_state(2);
        Filter filter1 = Filter_Builder(filter_functor_kb_state)
                                .withName("filter1")
                                .withParallelism(filter1_degree)
                                .withOutputBatchSize(dist_b(rng))
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
        ...
        Filter_Functor_KB filter_functor2(4);
        ...
        Filter_Functor_KB filter_functor3(5);
