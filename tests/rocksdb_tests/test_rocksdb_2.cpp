...
/*  
 *  Test 2 of general graphs that include persistent operators (using RocksDB).
 *  
 *                              +---------------------+
 *                              |  +-----+   +-----+  |
 *                           +->+  |  F  |   |  M  |  +-+
...
int main(int argc, char *argv[])
{
    ...
    int map1_degree, map2_degree, map3_degree, filter1_degree, filter2_degree, sink1_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "                            |  +-----+   +-----+  |" << endl;
        cout << "                         +->+  |  F  |   |  M  |  +-+" << endl;
        cout << "                         |  |  | (" << filter1_degree << ") +-->+ (" << map3_degree << ") |  | |" << endl;
        cout << "                         |  |  +-----+   +-----+  | |" << endl;
        cout << "+---------------------+  |  +---------------------+ |   +---------------------+" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   | P_F |  |  |                          |   |  |  M  |   |  S  |  |" << endl;
        cout << "|  | (" << source1_degree << ") +-->+ (" << map1_degree << ") |  +--+                          +-->+  | (" << map2_degree << ") +-->+ (" << sink1_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "+---------------------+  |       +-----------+      |   +---------------------+" << endl;
        cout << "                         |       |  +-----+  |      |" << endl;
        cout << "                         |       |  |  F  |  |      |" << endl;
        ...
        // prepare the test
        PipeGraph graph("test_rocksdb_2", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
...
#if 1
        P_Map_Functor_KB p_map_functor_kb;
        P_Map p_map1 = P_Map_Builder(p_map_functor_kb)
                                .withName("p_map1")
        ...
        pipe1.chain(p_map1);
#else
        Map_Functor_KB_State map_functor_kb_state;
        Map map1 = Map_Builder(map_functor_kb_state)
                                .withName("map1")
                                .withParallelism(map1_degree)
                                .withOutputBatchSize(dist_b(rng))
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
