...
/*  
 *  Test 3 of general graphs that include persistent operators (using RocksDB).
 *  
 *                              +---------------------+
 *                              |  +-----+   +-----+  |
 *                           +->+  |  F  |   |  M  |  +-+
...
int main(int argc, char *argv[])
{
    ...
    int map1_degree, map2_degree, flatmap_degree, filter1_degree, filter2_degree, sink1_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "+---------------------+  |  +---------------------+ |   +---------------------+" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   |P_FM |  |  |                          |   |  |  M  |   |  S  |  |" << endl;
        cout << "|  | (" << source1_degree << ") +-->+ (" << flatmap_degree << ") |  +--+                          +-->+  | (" << map1_degree << ") +-->+ (" << sink1_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "+---------------------+  |       +-----------+      |   +---------------------+" << endl;
        cout << "                         |       |  +-----+  |      |" << endl;
        cout << "                         |       |  |  F  |  |      |" << endl;
        ...
        // prepare the test
        PipeGraph graph("test_rocksdb_3", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
...
#if 1
        P_FlatMap_Functor_KB p_flatmap_functor_kb;
        P_FlatMap p_flatmap1 = P_FlatMap_Builder(p_flatmap_functor_kb)
                                .withName("p_flatmap1")
                                ...
                                .withDeleteDb(true)
                                .build();
        pipe1.chain(p_flatmap1);
#else
        FlatMap_Functor_KB_State flatmap_functor_kb_state;
        FlatMap flatmap1 = FlatMap_Builder(flatmap_functor_kb_state)
                                .withName("flatmap1")
                                .withParallelism(flatmap_degree)
                                .withOutputBatchSize(dist_b(rng))
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
                                .build();
        pipe1.chain(flatmap1);
#endif
        // split
