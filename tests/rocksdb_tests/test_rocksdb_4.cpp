/**************************************************************************************
 *  Copyright (c) 2019- Gabriele Mencagli
 *  
 *  This file is part of WindFlow.
 *  
 *  WindFlow is free software dual licensed under the GNU LGPL or MIT License.
 *  You can redistribute it and/or modify it under the terms of the
 *    * GNU Lesser General Public License as published by
 *      the Free Software Foundation, either version 3 of the License, or
 *      (at your option) any later version
 *    OR
 *    * MIT License: https://github.com/ParaGroup/WindFlow/blob/vers3.x/LICENSE.MIT
 *  
 *  WindFlow is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU Lesser General Public License for more details.
 *  You should have received a copy of the GNU Lesser General Public License and
 *  the MIT License along with WindFlow. If not, see <http://www.gnu.org/licenses/>
 *  and <http://opensource.org/licenses/MIT/>.
 **************************************************************************************
 */

/*  
 *  Test 4 of general graphs that include persistent operators (using RocksDB).
 *  
 *                              +---------------------+
 *                              |  +-----+   +-----+  |
 *                           +->+  |  F  |   |  M  |  +-+
 *                           |  |  | (*) +-->+ (*) |  | |
 *                           |  |  +-----+   +-----+  | |
 *  +---------------------+  |  +---------------------+ |   +---------------------+
 *  |  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |
 *  |  |  S  |   | P_R |  |  |                          |   |  |  M  |   |  S  |  |
 *  |  | (*) +-->+ (*) |  +--+                          +-->+  | (*) +-->+ (*) |  |
 *  |  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |
 *  +---------------------+  |       +-----------+      |   +---------------------+
 *                           |       |  +-----+  |      |
 *                           |       |  |  F  |  |      |
 *                           +------>+  | (*) |  +------+
 *                                   |  +-----+  |
 *                                   +-----------+
 */ 

// include
#include<random>
#include<iostream>
#include<windflow.hpp>
#include<persistent/windflow_rocksdb.hpp>
#include"rocksdb_common.hpp"

using namespace std;
using namespace wf;

// global variable for the result
extern atomic<long> global_sum;

// main
int main(int argc, char *argv[])
{
    int option = 0;
    size_t runs = 1;
    size_t stream_len = 0;
    size_t n_keys = 1;
    // initalize global variable
    global_sum = 0;
    // arguments from command line
    if (argc != 7) {
        cout << argv[0] << " -r [runs] -l [stream_length] -k [n_keys]" << endl;
        exit(EXIT_SUCCESS);
    }
    while ((option = getopt(argc, argv, "r:l:k:")) != -1) {
        switch (option) {
            case 'r': runs = atoi(optarg);
                     break;
            case 'l': stream_len = atoi(optarg);
                     break;
            case 'k': n_keys = atoi(optarg);
                     break;
            default: {
                cout << argv[0] << " -r [runs] -l [stream_length] -k [n_keys]" << endl;
                exit(EXIT_SUCCESS);
            }
        }
    }
    // set random seed
    mt19937 rng;
    rng.seed(std::random_device()());
    size_t min = 1;
    size_t max = 9;
    std::uniform_int_distribution<std::mt19937::result_type> dist_p(min, max);
    std::uniform_int_distribution<std::mt19937::result_type> dist_b(0, 10);
    int map1_degree, map2_degree, reduce_degree, filter1_degree, filter2_degree, sink1_degree;
    size_t source1_degree = 1;
    long last_result = 0;
    // executes the runs in DEFAULT mode
    for (size_t i=0; i<runs; i++) {
        map1_degree = dist_p(rng);
        map2_degree = dist_p(rng);
        reduce_degree = dist_p(rng);
        filter1_degree = dist_p(rng);
        filter2_degree = dist_p(rng);
        sink1_degree = dist_p(rng);
        cout << "Run " << i << endl;
        cout << "                            +---------------------+" << endl;
        cout << "                            |  +-----+   +-----+  |" << endl;
        cout << "                         +->+  |  F  |   |  M  |  +-+" << endl;
        cout << "                         |  |  | (" << filter1_degree << ") +-->+ (" << map2_degree << ") |  | |" << endl;
        cout << "                         |  |  +-----+   +-----+  | |" << endl;
        cout << "+---------------------+  |  +---------------------+ |   +---------------------+" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "|  |  S  |   | P_R |  |  |                          |   |  |  M  |   |  S  |  |" << endl;
        cout << "|  | (" << source1_degree << ") +-->+ (" << reduce_degree << ") |  +--+                          +-->+  | (" << map1_degree << ") +-->+ (" << sink1_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+  |  |                          |   |  +-----+   +-----+  |" << endl;
        cout << "+---------------------+  |       +-----------+      |   +---------------------+" << endl;
        cout << "                         |       |  +-----+  |      |" << endl;
        cout << "                         |       |  |  F  |  |      |" << endl;
        cout << "                         +------>+  | (" << filter2_degree << ") |  +------+" << endl;
        cout << "                                 |  +-----+  |" << endl;
        cout << "                                 +-----------+" << endl;
        // compute the total parallelism degree of the PipeGraph
        size_t check_degree = source1_degree;
        check_degree += reduce_degree;
        check_degree += filter1_degree;
        if (filter1_degree != map2_degree) {
            check_degree += map2_degree;
        }
        check_degree += filter2_degree;
        check_degree += map1_degree;
        if (map1_degree != sink1_degree) {
            check_degree += sink1_degree;
        }
        auto serializer = [](tuple_t &s) -> std::string {
                            return std::to_string(s.value) + "," + std::to_string(s.key);
        };
        auto deserializer = [](std::string &s) -> tuple_t {
            tuple_t state;
            state.value = atoi(s.substr(0, s.find(",")).c_str());
            state.key = atoi(s.substr(s.find(",")+1, s.length()-1).c_str());
            return state;
        };
        // prepare the test
        PipeGraph graph("test_rocksdb_4", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        // prepare the first MultiPipe
        Source_Functor source_functor(stream_len, n_keys, true);
        Source source = Source_Builder(source_functor)
                            .withName("source1")
                            .withParallelism(source1_degree)
                            .withOutputBatchSize(dist_b(rng))
                            .build();
        MultiPipe &pipe1 = graph.add_source(source);
#if 1
        P_Reduce_Functor_KB p_reduce_functor_kb;
        P_Reduce p_reduce1 = P_Reduce_Builder(p_reduce_functor_kb)
                                .withName("p_reduce1")
                                .withParallelism(reduce_degree)
                                .withOutputBatchSize(dist_b(rng))
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
                                .withSerializer(serializer)
                                .withDeserializer(deserializer)
                                .withDeleteDb(true)
                                .build();
        pipe1.add(p_reduce1);
#else
        Reduce_Functor_KB reduce_functor_kb;
        Reduce reduce1 = Reduce_Builder(reduce_functor_kb)
                                .withName("reduce1")
                                .withParallelism(reduce_degree)
                                .withOutputBatchSize(dist_b(rng))
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
                                .build();
        pipe1.add(reduce1);
#endif
        // split
        pipe1.split([](const tuple_t &t) {
            if (t.value % 2 == 0) {
                return 0;
            }
            else {
                return 1;
            }
        }, 2);
        // prepare the second MultiPipe
        MultiPipe &pipe2 = pipe1.select(0);
        Filter_Functor_KB filter_functor1(4);
        Filter filter1 = Filter_Builder(filter_functor1)
                                .withName("filter1")
                                .withParallelism(filter1_degree)
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
                                .withOutputBatchSize(dist_b(rng))
                                .build();
        pipe2.chain(filter1);
        Map_Functor map_functor2;
        Map map2 = Map_Builder(map_functor2)
                        .withName("map2")
                        .withParallelism(map2_degree)
                        .withOutputBatchSize(dist_b(rng))
                        .build();
        pipe2.chain(map2);
        // prepare the third MultiPipe
        MultiPipe &pipe3 = pipe1.select(1);
        Filter_Functor_KB filter_functor2(5);
        Filter filter2 = Filter_Builder(filter_functor2)
                                .withName("filter2")
                                .withParallelism(filter2_degree)
                                .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
                                .withOutputBatchSize(dist_b(rng))
                                .build();
        pipe3.chain(filter2);
        // pepare the fourth MultiPipe
        MultiPipe &pipe4 = pipe2.merge(pipe3);
        Map_Functor map1_functor;
        Map map1 = Map_Builder(map1_functor)
                        .withName("map1")
                        .withParallelism(map1_degree)
                        .withOutputBatchSize(dist_b(rng))
                        .build();
        pipe4.chain(map1);
        Sink_Functor sink_functor;
        Sink sink = Sink_Builder(sink_functor)
                        .withName("sink")
                        .withParallelism(sink1_degree)
                        .build();
        pipe4.chain_sink(sink);
        assert(graph.getNumThreads() == check_degree);
        // run the application
        graph.run();
        if (i == 0) {
            last_result = global_sum;
            cout << "Result is --> " << GREEN << "OK" << DEFAULT_COLOR << " value " << global_sum.load() << endl;
        }
        else {
            if (last_result == global_sum) {
                cout << "Result is --> " << GREEN << "OK" << DEFAULT_COLOR << " value " << global_sum.load() << endl;
            }
            else {
                cout << "Result is --> " << RED << "FAILED" << DEFAULT_COLOR << " value " << global_sum.load() << endl;
                abort();
            }
        }
        global_sum = 0;
    }
    return 0;
}
