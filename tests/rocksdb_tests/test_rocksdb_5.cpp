...
/*  
 *  Test 5 of general graphs that include persistent operators (using RocksDB).
 *  
 *                              +---------------------+
 *                              |  +-----+   +-----+  |
 *                           +->+  |  F  |   |  M  |  +-+
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        // prepare the test
        PipeGraph graph("test_rocksdb_5", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        ...
        Map map1 = Map_Builder(map1_functor)
                        ...
                        .build();
        pipe4.chain(map1);
        P_Sink_Functor p_sink_functor;
        P_Sink p_sink = P_Sink_Builder(p_sink_functor)
                            .withName("sink")
