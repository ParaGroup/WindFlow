...
/*  
 *  Test 6 of the P_Keyed_Windows operator with count-based windows and incremental
 *  window function.
 *  
 *  +-----------------------------------------------------------------+
 *  |  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        auto result_deserializer = [](std::string &s) -> result_t {
            result_t r;
            r.key = atoll(s.substr(0, s.find(",")).c_str());
            r.value = atoll(s.substr(s.find(",")+1, s.find(";")).c_str());
            r.wid = atoll(s.substr(s.find(";")+1, s.length()-1).c_str());
            return r;
        };
        // prepare the test
        PipeGraph graph("test_rocksdb_6", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
