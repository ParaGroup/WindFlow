...
/*  
 *  Test of the Keyed_Windows operator with count-based windows.
 *  
 *  +----------------------------------------------------------------+
 *  |  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |
 *  |  |  S  |   |  F  |   |  FM  |   |  M  |   | KW_CB |   |  S  |  |
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "+----------------------------------------------------------------+" << endl;
        cout << "|  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |" << endl;
        cout << "|  |  S  |   |  F  |   |  FM  |   |  M  |   | KW_CB |   |  S  |  |" << endl;
        cout << "|  | (" << source_degree << ") +-->+ (" << filter_degree << ") +-->+  (" << flatmap_degree << ") +-->+ (" << map_degree << ") +-->+  (" << kw_degree << ")  +-->+ (" << sink_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |" << endl;
        cout << "+----------------------------------------------------------------+" << endl;
        // prepare the test
        PipeGraph graph("test_win_kw_cb", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
