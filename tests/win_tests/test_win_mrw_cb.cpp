...
/*  
 *  Test of the MapReduce_Windows operator with count-based windows.
 *  
 *  +-----------------------------------------------------------------+
 *  |  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |
 *  |  |  S  |   |  F  |   |  FM  |   |  M  |   | MRW_CB |   |  S  |  |
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        std::cout << "+-----------------------------------------------------------------+" << std::endl;
        std::cout << "|  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |" << std::endl;
        std::cout << "|  |  S  |   |  F  |   |  FM  |   |  M  |   | MRW_CB |   |  S  |  |" << std::endl;
        std::cout << "|  | (" << source_degree << ") +-->+ (" << filter_degree << ") +-->+  (" << flatmap_degree << ") +-->+ (" << map_degree << ") +-->+  (" << wmap_degree << "," << reduce_degree << ") +-->+ (" << sink_degree << ") |  |" << std::endl;
        std::cout << "|  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |" << std::endl;
        std::cout << "+-----------------------------------------------------------------+" << std::endl;
        // prepare the test
        PipeGraph graph("test_win_mrw_cb", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
