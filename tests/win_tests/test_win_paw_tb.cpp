...
/*  
 *  Test of the Paned_Windows operator with time-based windows.
 *  
 *  +-----------------------------------------------------------------+
 *  |  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |
 *  |  |  S  |   |  F  |   |  FM  |   |  M  |   | PaW_TB |   |  S  |  |
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        std::cout << "+-----------------------------------------------------------------+" << std::endl;
        std::cout << "|  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |" << std::endl;
        std::cout << "|  |  S  |   |  F  |   |  FM  |   |  M  |   | PaW_TB |   |  S  |  |" << std::endl;
        std::cout << "|  | (" << source_degree << ") +-->+ (" << filter_degree << ") +-->+  (" << flatmap_degree << ") +-->+ (" << map_degree << ") +-->+  (" << plq_degree << "," << wlq_degree << ") +-->+ (" << sink_degree << ") |  |" << std::endl;
        std::cout << "|  +-----+   +-----+   +------+   +-----+   +--------+   +-----+  |" << std::endl;
        std::cout << "+-----------------------------------------------------------------+" << std::endl;
        // prepare the test
        PipeGraph graph("test_win_paw_tb", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        std::cout << "+-----------------------------------------------------------------+" << std::endl;
        // prepare the test
        PipeGraph graph("test_win_paw_tb", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
