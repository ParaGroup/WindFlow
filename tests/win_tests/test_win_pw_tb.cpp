...
/*  
 *  Test of the Parallel_Windows operator with time-based windows.
 *  
 *  +----------------------------------------------------------------+
 *  |  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |
 *  |  |  S  |   |  F  |   |  FM  |   |  M  |   | PW_TB |   |  S  |  |
...
int main(int argc, char *argv[])
{
    ...
    int filter_degree, flatmap_degree, map_degree, pw_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        pw_degree = dist_p(rng);
        sink_degree = dist_p(rng);
        cout << "Run " << i << endl;
        cout << "+----------------------------------------------------------------+" << endl;
        cout << "|  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |" << endl;
        cout << "|  |  S  |   |  F  |   |  FM  |   |  M  |   | PW_TB |   |  S  |  |" << endl;
        cout << "|  | (" << source_degree << ") +-->+ (" << filter_degree << ") +-->+  (" << flatmap_degree << ") +-->+ (" << map_degree << ") +-->+  (" << pw_degree << ")  +-->+ (" << sink_degree << ") |  |" << endl;
        cout << "|  +-----+   +-----+   +------+   +-----+   +-------+   +-----+  |" << endl;
        cout << "+----------------------------------------------------------------+" << endl;
        // prepare the test
        PipeGraph graph("test_win_pw_tb", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        cout << "+----------------------------------------------------------------+" << endl;
        // prepare the test
        PipeGraph graph("test_win_pw_tb", Execution_Mode_t::DETERMINISTIC, Time_Policy_t::EVENT_TIME);
