...
/*  
 *  Test of the Parallel_Windows operator with time-based windows. This test
...
int main(int argc, char *argv[])
{
    ...
    std::uniform_int_distribution<std::mt19937::result_type> dist_b(50, 50);
    int filter_degree, map1_degree, map2_degree, map3_degree, pw_degree;
    ...
    for (size_t i=0; i<runs; i++) {
        ...
        map1_degree = 1; dist_p(rng);
        map2_degree = 1; dist_p(rng);
        map3_degree = 1; dist_p(rng);
        pw_degree = 1; dist_p(rng);
        sink_degree = 1; dist_p(rng);
        cout << "Run " << i << endl;
        std::cout << "+-------------------------------------------------------------------------+" << std::endl;
        std::cout << "|  +-----+     +-----+     +-----+     +-----+     +-------+     +-----+  |" << std::endl;
        std::cout << "|  |  S  |     |  F  |     |  M  |     |  M  |     | PW_TB |     |  S  |  |" << std::endl;
        std::cout << "|  | CPU +---->+ CPU +---->+ GPU +---->+ GPU +---->+  CPU  +---->+ CPU |  |" << std::endl;
        std::cout << "|  | (" << source_degree << ") |     | (" << filter_degree << ") |     | (" << map1_degree << ") |     | (" << map2_degree << ") |     |  (" << pw_degree << ")  |     | (" << sink_degree << ") |  |" << std::endl;
        std::cout << "|  +-----+     +-----+     +-----+     +-----+     +-------+     +-----+  |" << std::endl;
        std::cout << "+-------------------------------------------------------------------------+" << std::endl;
        // prepare the test
        PipeGraph graph("test_win_pw_gpu_tb", Execution_Mode_t::DEFAULT, Time_Policy_t::EVENT_TIME);
        ...
        Parallel_Windows pwins = Parallel_Windows_Builder(win_functor)
                                    .withName("par_wins")
                                    .withParallelism(pw_degree)
                                    .withKeyBy([](const tuple_t &t) -> size_t { return t.key; })
