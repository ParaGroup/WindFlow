        ...
        if (input_batching) { // receiving a batch
            ...
            if (batch_input->isPunct()) { // if it is a punctuaton
                ...
                if (execution_mode == Execution_Mode_t::DEFAULT) { // useless check ?
                    assert(last_time <= batch_input->getWatermark(context.getReplicaIndex())); // redundant check (watermarks are always monotonically increasing)
            ...
            if (input->isPunct()) { // if it is a punctuaton
                ...
                if (execution_mode == Execution_Mode_t::DEFAULT) { // useless check ?
                    assert(last_time <= input->getWatermark(context.getReplicaIndex())); // redundant check (watermarks are always monotonically increasing)
