 ...
 *  windows efficiently on a GPU device.
 *  
...
private:
    ...
    result_t *pinned_buffer = nullptr; // host pinned array of general use
    result_t *pinned_results = nullptr; // host pinned array of the final results
    result_t *pinned_init_results = nullptr; // host pinned array of the initial results
    result_t *gpu_tree = nullptr; // GPU array of the whole tree
    result_t *gpu_results = nullptr; // GPU array of the final results
...
public:
                ...
                cudaStream(_cudaStream)
    {
        ...
        gpuErrChk(cudaMallocHost(&pinned_buffer, treeSizeBytes));
        gpuErrChk(cudaMallocHost(&pinned_results, numWinsPerBatch * sizeof(result_t)));
        gpuErrChk(cudaMallocHost(&pinned_init_results, numWinsPerBatch * sizeof(result_t)));
        gpuErrChk(cudaMalloc(&gpu_tree, treeSizeBytes));
        gpuErrChk(cudaMalloc(&gpu_results, numWinsPerBatch * sizeof(result_t)));
        std::vector<result_t> tmp_vect;
        tmp_vect.resize(numWinsPerBatch, create_win_result_t<result_t, key_t>(key, 0));
        memcpy(pinned_init_results, tmp_vect.data(), numWinsPerBatch * sizeof(result_t));
    }
                ...
                cudaStream(_other.cudaStream)
    {
        ...
        gpuErrChk(cudaMalloc(&gpu_tree, treeSizeBytes));
        gpuErrChk(cudaMalloc(&gpu_results, numWinsPerBatch * sizeof(result_t)));
        memcpy(pinned_init_results, _other.pinned_init_results, numWinsPerBatch * sizeof(result_t));
    }
    ...
    FlatFAT_GPU(FlatFAT_GPU &&_other):
                ...
                offset(_other.offset),
                pinned_buffer(std::exchange(_other.pinned_buffer, nullptr)),
                pinned_results(std::exchange(_other.pinned_results, nullptr)),
                pinned_init_results(std::exchange(_other.pinned_init_results, nullptr)),
                gpu_tree(std::exchange(_other.gpu_tree, nullptr)),
                gpu_results(std::exchange(_other.gpu_results, nullptr)),
                numSMs(_other.numSMs),
                max_blocks_per_sm(_other.max_blocks_per_sm),
    ...
    ~FlatFAT_GPU()
    {
        ...
        if (pinned_results != nullptr) {
            cudaFreeHost(pinned_results);
        }
        if (pinned_init_results != nullptr) {
            cudaFreeHost(pinned_init_results);
        }
        if (gpu_tree != nullptr) {
            cudaFree(gpu_tree);
        }
        if (gpu_results != nullptr) {
               ...
               uint64_t _initial_gwid)
    {
        ...
        memcpy(pinned_buffer, tree.data(), treeSizeBytes); // copy the tree vector in the pinned array
        gpuErrChk(cudaMemcpyAsync(gpu_tree, pinned_buffer, treeSizeBytes, cudaMemcpyHostToDevice, *cudaStream)); // copy on GPU
        ...
        while (d_levelB < gpu_tree + treeSize && i > 0) { // fill the levels of the tree
            int numBlocks = std::min((int) ceil(i / ((double) WF_DEFAULT_THREADS_PER_BLOCK)), numSMs * max_blocks_per_sm);
            Init_TreeLevel_Kernel<result_t, combgpu_func_t><<<numBlocks, WF_DEFAULT_THREADS_PER_BLOCK, 0, *cudaStream>>>(comb_func,
                                                                                                                         d_levelA,
                                                                                                                         d_levelB,
                                                                                                                         i);
        ...
        gpuErrChk(cudaMemcpyAsync(gpu_results, pinned_init_results, numWinsPerBatch * sizeof(result_t), cudaMemcpyHostToDevice, *cudaStream)); // copy the initial values in the GPU
        int numBlocks = std::min((int) ceil(numWinsPerBatch / ((double) WF_DEFAULT_THREADS_PER_BLOCK)), numSMs * max_blocks_per_sm); // compute the results of the first batch
        Compute_Results_Kernel<key_t, result_t, combgpu_func_t><<<numBlocks, WF_DEFAULT_THREADS_PER_BLOCK, 0, *cudaStream>>>(comb_func,
                                                                                                                             key,
                                                                                                                             gpu_tree,
                                                                                                                             gpu_results,
                                                                                                                             _initial_gwid,
                ...
                uint64_t _initial_gwid)
    {
        ...
        if (inputs.size() <= spaceLeft) {
            memcpy(pinned_buffer, inputs.data(), inputs.size() * sizeof(result_t)); // copy the inputs in the pinned array
            gpuErrChk(cudaMemcpyAsync(gpu_tree + offset, pinned_buffer, inputs.size() * sizeof(result_t), cudaMemcpyHostToDevice, *cudaStream)); // copy on GPU
        }
        else { // split the copy on GPU in two parts
            memcpy(pinned_buffer, inputs.data(), inputs.size() * sizeof(result_t)); // copy the inputs in the page-locked memory
        ...
        while (d_levelB < gpu_tree + treeSize) { // update the levels of the tree, each with a separate kernel
            ...
            size_t numBlocks = std::min((int) ceil(sizeUpdate / ((double) WF_DEFAULT_THREADS_PER_BLOCK)), numSMs * max_blocks_per_sm);
            Update_TreeLevel_Kernel<result_t, combgpu_func_t><<<numBlocks, WF_DEFAULT_THREADS_PER_BLOCK, 0, *cudaStream>>>(comb_func,
                                                                                                                           d_levelA,
                                                                                                                           d_levelB,
                                                                                                                           distance,
                                                                                                                           sizeB,
        ...
        gpuErrChk(cudaMemcpyAsync(gpu_results, pinned_init_results, numWinsPerBatch * sizeof(result_t), cudaMemcpyHostToDevice, *cudaStream)); // copy the initial values in the GPU
        int numBlocks = std::min((int) ceil(numWinsPerBatch / ((double) WF_DEFAULT_THREADS_PER_BLOCK)), numSMs * max_blocks_per_sm);
        Compute_Results_Kernel<key_t, result_t, combgpu_func_t><<<numBlocks, WF_DEFAULT_THREADS_PER_BLOCK, 0, *cudaStream>>>(comb_func,
                                                                                                                             key,
                                                                                                                             gpu_tree,
                                                                                                                             gpu_results,
    ...
    {
        gpuErrChk(cudaMemcpyAsync(pinned_buffer, (gpu_tree + offset), (batchSize - offset) * sizeof(result_t), cudaMemcpyDeviceToHost, *cudaStream));
        gpuErrChk(cudaMemcpyAsync((pinned_buffer + batchSize - offset), gpu_tree, offset * sizeof(result_t), cudaMemcpyDeviceToHost, *cudaStream));
        gpuErrChk(cudaStreamSynchronize(*cudaStream));
