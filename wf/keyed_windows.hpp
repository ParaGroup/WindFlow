...
/** 
 *  @file    keyed_windows.hpp
 *  @author  Gabriele Mencagli
 *  
 *  @brief Keyed_Windows operator
 *  
 *  @section Keyed_Windows (Description)
 *  
 *  This file implements the Keyed_Windows operator able to execute incremental
 *  or non-incremental processing logic on count- or time-based windows. If
 *  created with parallelism greater than one, windows belonging to different
 *  keys are processed in parallel by different operator replicas.
 */ 
 ...
 *  \brief Keyed_Windows operator
 *  
 *  This class implements the Keyed_Windows operator executing incremental or
 *  non-incremental processing logic on streaming windows. The operator allows
 *  windows of different keys to be processed in parallel by different replica.
 */ 
template<typename win_func_t, typename key_extractor_func_t>
class Keyed_Windows: public Basic_Operator
{
private:
    ...
    size_t parallelism; // parallelism of the Keyed_Windows
    std::string name; // name of the Keyed_Windows
    bool input_batching; // if true, the Keyed_Windows expects to receive batches instead of individual inputs
    size_t outputBatchSize; // batch size of the outputs produced by the Keyed_Windows
    std::vector<Window_Replica<win_func_t, key_extractor_func_t>*> replicas; // vector of pointers to the replicas of the Keyed_Windows
    ...
    // Set the emitter used to route outputs from the Keyed_Windows
    ...
    // Check whether the Keyed_Windows has terminated
    ...
    // Set the execution mode of the Keyed_Windows (i.e., the one of its PipeGraph)
...
#if defined (WF_TRACING_ENABLED)
    // Dump the log file (JSON format) of statistics of the Keyed_Windows
...
public:
     ...
     *  \param _parallelism internal parallelism of the Keyed_Windows
     *  \param _name name of the Keyed_Windows
     *  \param _outputBatchSize size (in num of tuples) of the batches produced by this operator (0 for no batching)
     *  \param _closing_func closing functional logic of the Keyed_Windows (a function or callable type)
     *  \param _win_len window length (in no. of tuples or in time units)
     *  \param _slide_len slide length (in no. of tuples or in time units)
     *  \param _lateness (lateness in time units, meaningful for TB windows in DEFAULT mode)
     *  \param _winType window type (count-based CB or time-based TB)
     */ 
    Keyed_Windows(win_func_t _func,
                  ...
                  winType(_winType)
    {
        if (parallelism == 0) { // check the validity of the parallelism value
            std::cerr << RED << "WindFlow Error: Keyed_Windows has parallelism zero" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        if (win_len == 0 || slide_len == 0) { // check the validity of the windowing parameters
            std::cerr << RED << "WindFlow Error: Keyed_Windows used with window length or slide equal to zero" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        for (size_t i=0; i<parallelism; i++) { // create the internal replicas of the Keyed_Windows
    ...
    /// Copy constructor
    Keyed_Windows(const Keyed_Windows &_other):
                  func(_other.func),
    ...
    Keyed_Windows& operator=(const Keyed_Windows &_other)
    {
        if (this != &_other) {
            func = _other.func;
            key_extr = _other.key_extr;
    ...
    Keyed_Windows& operator=(Keyed_Windows &&_other)
    {
    ...
    /** 
     *  \brief Get the type of the Keyed_Windows as a string
     *  \return type of the Keyed_Windows
     */ 
    std::string getType() const override
    {
        return std::string("Keyed_Windows");
    }

    /** 
     *  \brief Get the name of the Keyed_Windows as a string
     *  \return name of the Keyed_Windows
     */ 
    ...
    /** 
     *  \brief Get the total parallelism of the Keyed_Windows
     *  \return total parallelism of the Keyed_Windows
     */  
    ...
    /** 
     *  \brief Return the input routing mode of the Keyed_Windows
     *  \return routing mode used to send inputs to the Keyed_Windows
     */ 
    ...
    /** 
     *  \brief Return the size of the output batches that the Keyed_Windows should produce
    ...
    /** 
     *  \brief Get the window type (CB or TB) utilized by the Keyed_Windows
    ...
    /** 
     *  \brief Get the number of ignored tuples by the Keyed_Windows
     *  \return number of tuples ignored during the processing by the Keyed_Windows
     */ 
